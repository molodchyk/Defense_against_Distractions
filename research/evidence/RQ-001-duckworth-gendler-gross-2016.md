# Evidence Card

## Source

Citation: Duckworth, A. L., Gendler, T. S., & Gross, J. J. (2016). Situational Strategies for Self-Control. Perspectives on Psychological Science, 11(1), 35-55.

Link: https://pmc.ncbi.nlm.nih.gov/articles/PMC4736542/

DOI: https://doi.org/10.1177/1745691615623247

## Source Type

- review / theoretical framework
- psychology

## Research Context

The paper organizes self-control strategies by when they intervene in the unfolding of a temptation or conflict. It emphasizes situation selection and situation modification as proactive strategies that can prevent self-control conflicts from becoming acute.

## Main Finding

Self-control is not only response inhibition. People can support self-control by choosing or changing situations before temptation fully develops. Earlier-stage strategies can reduce reliance on brute force willpower.

## Limitations

The paper is a conceptual review, not a direct test of browser extension locks. It supports DaD's design philosophy more than a specific threshold or schedule rule.

## Evidence Grade

strong for the conceptual model; moderate for direct DaD implementation.

## Relevance To DaD

Locked schedules are situation modification. Instead of expecting the user to resist a blocked site during a vulnerable state, DaD changes the browser environment before that moment arrives.

The "stricter allowed, relaxation blocked" rule also fits this model: changes that reduce temptation are allowed, while changes that reintroduce temptation are delayed until the user is outside the high-risk situation.

## Product Implication

DaD should frame locked schedules as environmental design, not moral discipline. The UI should say roughly:

"This schedule is active. You can add protection now, but relaxing this plan waits until the protected period ends."

## Notes

This is the strongest psychology source for DaD's general defense model.
