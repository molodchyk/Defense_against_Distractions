...
- Load the extension zip or unpacked project in an isolated Chromium-based browser/profile where testing cannot close active user windows or unsaved work.
...
- Open a plan's Actions tab, create a triggered action from one or two existing UI cleanup rules, add an optional absent-target guard, confirm the summary shows the ordered action sequence and absence condition, confirm it can use any block score or a specific keyword/structural trigger, confirm it appears in the plan fact count, and during an active locked schedule confirm action-only chains cannot be added or enabled while block-after-action chains remain allowed.
- Open the popup Inspect pane on the target page and confirm the Preview panel lists active saved triggered actions with current-tab target/action/block status before any action runs.
- Trigger that plan action on a harmless test page and confirm popup Block Diagnostics shows the latest action outcome, such as cleanup ran or fallback block used, without page text or full URLs.
