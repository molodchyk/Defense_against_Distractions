...
- Open a plan's Actions tab, create a triggered action from one or two existing UI cleanup rules, confirm the summary shows the ordered action sequence, confirm it can use any block score or a specific keyword/structural trigger, confirm it appears in the plan fact count, and during an active locked schedule confirm action-only chains cannot be added or enabled while block-after-action chains remain allowed.
