...
- Completed checkpoint: Chrome sync/local storage wrappers and extension-page/background storage-change listener registration now live under `src/platform/chrome/`, plan-critical storage priority lives under `src/features/plans/storage/`, and old shared storage paths are compatibility barrels.
...
- Chrome sync/local storage wrappers and extension-page/background storage-change listener registration now live under `src/platform/chrome/`, plan-critical storage priority lives under `src/features/plans/storage/`, and shared storage paths are compatibility barrels. Shared UI helpers live under `src/js/shared/ui/`.
