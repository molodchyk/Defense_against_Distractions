...
DaD Select quick add is a related creation shortcut: selected page text can become a keyword with an editable score estimate and optional safe action presets. Its action-chain preset saves or confirms the selected keyword and opens the selected plan's Actions editor with that keyword preselected as the draft trigger when the compact filter editor can represent it, while ordered steps, scenario guards, and fallbacks still belong in this triggered-action editor. See [DaD Select quick add](selected-text-quick-add.md).
