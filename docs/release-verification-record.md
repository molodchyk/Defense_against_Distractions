...
| --- | --- |
| `npm test` | Passed directly and through `npm run verify:release`; 562 unit tests passed on 2026-07-06. |
| `npm run verify:manifest` | Covered by `npm run verify:release`; 85 manifest-referenced paths exist. |
| `npm run verify:imports` | Passed directly and covered by `npm run verify:release`; 402 relative source files scanned. |
...
| `npm run audit:file-sizes` | Passed directly and through `npm run verify:release`; 426 budgeted files checked and no files exceed documented budgets. |
...
| `npm run verify:package` | Passed directly and through `npm run verify:release`; 361 packaged runtime files scanned and 85 manifest references verified. |
...
Notes: Keep npm run verify:browser-load separate; only run it in a safe isolated browser/profile. Popup triggered-action preview, the plan Actions draft outline, and the DaD Select action-chain preset with keyword-trigger prefill were added after the broad manual QA pass and should be checked manually before publishing.
```
