...
`src/features/triggered-actions/core/` owns the pure triggered-action chain model. It defines bounded current-page chain configuration, scenario guard selection, step/fallback normalization, local outcome event shape, and protected-schedule strictness classification before any live keyword-trigger integration uses it.
