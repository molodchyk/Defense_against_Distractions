...
- `src/platform/chrome/tabs.js` owns popup tab query, creation, messaging, URL update, and tab lifecycle listener helpers; popup/background modules call it instead of raw `chrome.tabs` callbacks.
- `src/platform/chrome/windows.js` owns window focus-change listener registration and no-focused-window id access.
