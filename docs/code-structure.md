...
`src/features/triggered-actions/core/` owns the pure triggered-action chain model. It defines bounded current-page chain configuration, scenario guard selection, step/fallback normalization, local outcome event shape, protected-schedule strictness classification, and the simple editor compiler used by Options. Plan records can carry these chains as durable protection configuration. `src/js/content/triggered-actions/runner.js` is the classic content-script executor that runs active plan-owned current-page chains against existing picker-created UI rule targets before the normal keyword block overlay. `src/js/options/plans/actionEditor.js` owns the first compact plan Actions tab for authoring simple target-backed chains.
...
- `src/js/options/plans/actionEditor.js` owns the compact plan Actions tab that creates and lists simple plan-owned triggered action chains backed by existing UI cleanup rules.
