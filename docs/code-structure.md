...
`src/features/triggered-actions/core/` owns the pure triggered-action chain model. It defines bounded current-page chain configuration, scenario guard selection, step/fallback normalization, local outcome event shape, and protected-schedule strictness classification. Plan records can carry these chains as durable protection configuration. `src/js/content/triggered-actions/runner.js` is the classic content-script executor that runs active plan-owned current-page chains against existing picker-created UI rule targets before the normal keyword block overlay; the full editor UI remains follow-up work.
