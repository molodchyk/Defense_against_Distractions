...
`src/features/content-blocking/background/runtime.js` owns background message routing and tab lifecycle hooks for page blocking, including badge update policy, top-frame block requests, and mute-state delegation through platform wrappers. `src/features/content-blocking/background/tabMute.js` owns blocked-page tab mute state for that runtime while `src/platform/chrome/tabs.js` owns tab lookup and mute-state updates. `src/features/content-blocking/blocked-page/` owns the extension blocked-page modules for the Chrome-platform facade, localization including right-to-left document direction, theme sync, custom message rendering, and Pomodoro timer rendering.
...
`src/features/triggered-actions/core/` owns the pure triggered-action chain model. It defines bounded current-page chain configuration, scenario guard selection, step/fallback normalization, local outcome event shape, protected-schedule strictness classification, and the simple editor compiler used by Options. The compiler can build one primary target-backed scenario plus one bounded alternative scenario for the same trigger, while richer scenario authoring remains outside the current compact UI. `strictness.js` owns protected-schedule comparison for chain edits so trigger narrowing, step weakening, fallback weakening, and run-policy relaxation stay centralized. Plan records can carry these chains as durable protection configuration. `src/js/content/triggered-actions/runner.js` is the classic content-script executor that runs active plan-owned current-page chains against existing picker-created UI rule targets before the normal keyword block overlay and owns the dry-run preview contract for checking current-page scenario and target availability without applying actions or blocking. `src/js/content/triggered-actions/previewMessage.js` owns the content-runtime message endpoint for that preview so the app entry stays wiring-only. `src/js/options/plans/actionEditor.js` owns the first compact plan Actions tab for authoring simple target-backed chains with optional keyword or structural trigger filters, one bounded alternative scenario, and validated DaD Select trigger prefill. `src/js/options/deepLinks.js` owns options hash parsing for selected plan pages and one-shot action-draft parameters. `src/js/options/plans/actionEditorSummary.js` owns action-editor option lists, chain summaries, and strictness-facing block-after-summary helpers. `src/js/options/plans/actionEditorTriggers.js` owns plan-derived keyword and structural-token suggestions for the action trigger filter.

`src/platform` contains browser/platform adapters that are not product features. `src/platform/chrome/storage.js` owns Promise wrappers around `chrome.storage.sync` and `chrome.storage.local`, sync quota access, plus storage-change listener registration for extension pages, options modules, and background modules; it is the default import for extension pages, options modules, background modules, shared UI helpers, and feature storage policies that need Chrome storage. `src/platform/chrome/contentBridge.js` owns the classic content-script bridge for runtime messages, runtime message listeners, extension URLs, storage, storage quota, storage-change listeners, and i18n because manifest-loaded content scripts are not ES modules. `src/platform/chrome/action.js` owns toolbar action click listener registration, badge text updates, and guarded action popup opening. `src/platform/chrome/alarms.js` owns Chrome alarm creation, clearing, and alarm listener registration for background timer wakeups. `src/platform/chrome/contextMenus.js` owns context-menu creation, removal, and click-listener registration. `src/platform/chrome/idle.js` owns Chrome idle detection interval, state-change listener registration, and initial state queries for Pomodoro rest credit. `src/platform/chrome/i18n.js` owns localized message and browser UI-language access. `src/platform/chrome/navigation.js` owns webNavigation committed and history-state listener registration. `src/platform/chrome/downloads.js` owns the Promise wrapper around `chrome.downloads.download` for user-triggered export flows. `src/platform/chrome/runtime.js` owns Chrome runtime lifecycle listener registration, message-listener registration, manifest lookup, extension URL generation, extension-context availability, and Options-page opening. `src/platform/chrome/runtimeMessages.js` owns Promise-based `chrome.runtime.sendMessage` access for extension pages that ask the background runtime for diagnostics or timer state. `src/platform/chrome/tabs.js` owns Promise-based tab query, tab lookup, tab creation, tab messaging, tab URL update, tab removal, tab moving, tab discard, tab mute updates, and tab lifecycle listener helpers. `src/platform/chrome/windows.js` owns window focus listener registration, window creation, and the browser no-focused-window id.
...
Project validation scripts live under `scripts/`. `scripts/check-browser-extension-playbook.mjs` owns repository-level playbook verification, while `scripts/playbook/` owns focused verifier support modules shared by that check so the verifier can stay below the documented script-size budget. Store-facing verifier helpers live under `scripts/playbook/store/` for StorePilot automation and store-media review checks instead of flattening those responsibilities into the root playbook support folder.
...
`src/js/popup` contains popup-only helper modules. The app entry `src/app/popup/index.js` bootstraps these helpers: it initializes language/theme state, creates the panel set, starts refresh loops, handles storage changes, and binds events. Generic Chrome messaging, DOM helpers, formatting helpers, popup shell behavior, popup i18n, panel construction, UI picker launching, diagnostics export, event binding, refresh intervals, the protection summary, the Page Signals panel, the Block Diagnostics panel, triggered-action preview, and the Pomodoro panel live in focused modules under `src/js/popup/`. `popup/pageSignalsPanel.js` owns current-page signal counts, passive-region summaries, ephemeral keyword-idea derivation and copy formatting from bounded page-signal tokens, plus selected-text candidate display using the shared selected-text candidate helper. `popup/triggered-actions/previewPanel.js` owns active-plan chain collection, non-mutating current-tab preview messaging, bounded preview formatting, and compact diagnostics export for saved triggered actions. `popup/quick-add/selectedTextQuickAddModel.js` owns the pure DaD Select plan mutation rules, keyword/action-preset compile contract, block-page score forcing, and picker-scope requirement for UI cleanup presets. `popup/quick-add/selectedTextQuickAddPanel.js` owns the popup plan/entry/action/score controls, protected-schedule guard, prioritized plan save, picker-backed cleanup preset launch, action-chain deep-link launch, right-click pending-candidate consumption, and current-tab re-check request. `popup/shell.js` owns the Control/Inspect tablist state, local pane persistence, keyboard navigation, theme loading, and storage reactions that affect the whole popup shell. Popup intent UI lives under `src/js/popup/intent/` for diagnostics, compact score-signal breakdowns, and recovery actions. Popup usage summaries live under `src/js/popup/usage/` and render in the Inspect pane with other read-only diagnostics. The Inspect action bar owns local diagnostics copy and copy-then-feedback actions; feedback opens externally only after the local copy succeeds. Future popup work should go into the narrowest matching popup module instead of expanding the entry file.

`src/css/popup.css` is a thin popup stylesheet entry point. It imports focused files from `src/css/popup`: `tokens.css` for theme variables, `layout.css` for shell/card/tab structure, `controls.css` for fields and buttons, `status.css` for protection badges and status grids, `pomodoro.css` for the popup timer panel, `diagnostics.css` for Page Signals, block diagnostics, triggered-action preview, usage summaries, and intent diagnostics, and `intent-recovery.css` for the Session coherence path and accountable Continue control. Future popup styling should go into the narrowest matching CSS module instead of growing the entry file.
...
- `src/features/plans/core/model.js` owns the tested plan model, normalization, default plan creation, allowed-site checks, triggered-action chain normalization, and next-name generation used by options and background code.
- `src/features/plans/core/protectedScheduleChanges.js` owns the pure strictness policy for plan edits during locked schedules, including Pomodoro, intent, UI rule, and triggered-action chain relaxations. Options UI should call through this shared comparator instead of duplicating protected-schedule edit rules.
- `src/js/shared/plans.js` is the compatibility barrel for current imports.

Chrome storage helpers now have explicit owners:

- `src/platform/chrome/storage.js` owns Promise wrappers around `chrome.storage.sync` and `chrome.storage.local`, sync quota access, plus storage-change listener registration for extension pages, options modules, and background modules.
- `src/platform/chrome/contentBridge.js` owns classic content-script runtime messages, message listeners, extension URLs, sync/local storage, sync quota, storage-change listeners, and i18n access; content scripts call `window.DAD.ChromePlatform` or the compatibility `window.DAD.safe*` helpers instead of raw Chrome callbacks.
- `src/platform/chrome/action.js` owns toolbar action click listener registration, badge text updates, and guarded action popup opening.
...
- `src/platform/chrome/contextMenus.js` owns Chrome context-menu creation, removal, click-listener registration, and runtime error handling for the DaD Select right-click entry point.
...
- `src/platform/chrome/i18n.js` owns localized message lookup and browser UI-language access.
- `src/platform/chrome/navigation.js` owns webNavigation committed and history-state listener registration for intent chain lineage.
...
- `src/platform/chrome/runtime.js` owns runtime install/startup/message listener registration, manifest lookup, extension URL generation, extension-context availability, and Options-page opening.
...
- `src/platform/chrome/tabs.js` owns tab query, lookup, creation, messaging, URL update, removal, moving, discard, mute updates, and tab lifecycle listener helpers; popup/background modules call it instead of raw `chrome.tabs` callbacks.
- `src/platform/chrome/windows.js` owns window focus-change listener registration, window creation, and no-focused-window id access.
...
- `src/js/options/plans/actionEditor.js` owns the compact plan Actions tab that creates and lists simple plan-owned triggered action chains backed by existing UI cleanup rules, including trigger-source and keyword/structural-token filters, validated DaD Select keyword prefill, one bounded alternative scenario, and the live draft outline.
- `src/js/options/deepLinks.js` owns parsing options hash links, including the bounded plan action-draft trigger fields used by DaD Select.
...
- `src/js/content/triggered-actions/constants.js`: classic content-script constants for triggered action trigger, step, result, and element-action mapping names.
- `src/js/content/triggered-actions/utils.js`: classic content-script host, timestamp, id, text, and comparable-key helpers for triggered action runtime modules.
- `src/js/content/triggered-actions/model.js`: classic content-script normalization, trigger matching, scenario guard context, and deterministic scenario selection for active plan-owned chains.
- `src/js/content/triggered-actions/runner.js`: current-page triggered-action execution and dry-run preview calculation against existing UI rule targets before the normal block overlay.
- `src/js/content/triggered-actions/previewMessage.js`: content runtime message endpoint for non-mutating current-page chain preview.
...
- `src/js/content/page-signals/selectionCandidate.js` owns bounded active-selection extraction for DaD Select quick add, including candidate text caps, editable-field detection, token extraction, and an initial 0-100 score estimate without storing popup-path unsaved selected text.
- `src/features/selected-text/candidate.js` owns module-based selected-text candidate normalization, token extraction, score estimation, and popup copy formatting for the popup and background selected-text handoff.
...
- `src/js/background/intent/chromeApi.js` is the background intent adapter over `src/platform/chrome/storage.js`, `runtime.js`, `tabs.js`, and `windows.js`. It owns intent-specific result shaping for diagnostics-page opening, tab pressure, open-tab enumeration, tab URL update, tab discard, tab window moving, and tab removal while the platform wrappers own raw Chrome callbacks and `runtime.lastError` handling.
...
- `src/js/background/selectedTextQuickAdd.js` owns the DaD Select context-menu runtime: registering the selection-only menu item, turning explicit selected text into bounded local pending state, opening the popup best-effort, and answering the popup consume message through platform wrappers.
...
- `src/app/popup/index.js` is the compact popup bootstrap. Popup features are split under `src/js/popup/`: `chrome.js` uses platform storage, runtime-message, and tab wrappers for popup sync reads, background messages, Options, feedback, and local diagnostics graph opening; `shell.js` uses the platform storage wrapper for local pane persistence and shell-level theme behavior; `panelSet.js` assembles the panel modules; `elementPickerLauncher.js` starts the UI picker through the platform tab-message wrapper; `diagnosticsExport.js` builds/copies the Inspect-pane local diagnostics snapshot; `events.js` binds popup actions; `refreshLoop.js` owns polling intervals; and the existing panel modules render protection, Page Signals, block diagnostics, and Pomodoro status. `pageSignalsPanel.js` owns the pure current-page keyword-idea helper and copy actions while delegating selected-text candidate formatting to `src/features/selected-text/candidate.js`. `quick-add/selectedTextQuickAddModel.js` owns DaD Select plan mutation, action-preset compilation, and the rule that UI cleanup presets require a picker scope; `quick-add/selectedTextQuickAddPanel.js` owns the popup plan, entry, action, score, save controls, right-click pending-candidate consumption, and current-tab site re-check request. Popup intent behavior lives under `src/js/popup/intent/`: `intentDiagnosticsPanel.js` owns debug-state rendering, `intentRecoveryModel.js` owns pure session-coherence, compact timeline, active-intervention, and effective-policy display helpers, `intentRecoveryTimeline.js` owns compact timeline DOM rendering, `intentContinueControl.js` owns popup Continue availability and bounded reason-control state, `intentDriftTabActions.js` owns repeated drift-tab return/move/suspend/close runtime actions, and `intentRecoveryPanel.js` owns the main session-coherence card plus Return chain, Return, Continue, Isolate, Mark coherent, Show graph, and action orchestration.
...
- `src/js/background/pomodoro/chromeStorage.js` uses `src/platform/chrome/storage.js` for Chrome storage access and `src/platform/chrome/alarms.js` for alarm scheduling. It owns Pomodoro runtime/activity/history persistence.
...
- `src/js/background/pomodoro/initializer.js` owns Pomodoro background startup, idle detection startup through `src/platform/chrome/idle.js`, alarm wakeups through `src/platform/chrome/alarms.js`, runtime message routing through `src/platform/chrome/runtime.js`, and tab/window activity recording through `src/platform/chrome/tabs.js` and `src/platform/chrome/windows.js`.
...
- `src/app/blocked/index.js` is the blocked-page module entry. Blocked-page Chrome-platform facade, localization including right-to-left document direction, theme sync, custom message rendering, and Pomodoro timer rendering live under `src/features/content-blocking/blocked-page/`; the facade and localization module use `src/platform/chrome/storage.js`, `runtime.js`, `runtimeMessages.js`, and `i18n.js` instead of raw Chrome callbacks.
