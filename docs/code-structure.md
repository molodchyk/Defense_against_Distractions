...
- `src/platform/chrome/action.js` owns toolbar action click listener registration.
...
- `src/platform/chrome/tabs.js` owns tab query, creation, messaging, URL update, removal, moving, discard, and tab lifecycle listener helpers; popup/background modules call it instead of raw `chrome.tabs` callbacks.
