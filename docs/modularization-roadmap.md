...
- Classic content-script runtime messages, message listeners, extension URLs, storage, quota reads, storage-change listeners, and i18n now go through `src/platform/chrome/contentBridge.js`; content feature modules own page behavior while the platform bridge owns raw Chrome callbacks for manifest-loaded scripts.
