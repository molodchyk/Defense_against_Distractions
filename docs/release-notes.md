...
- Plans now expose an `Actions` tab for the first compact triggered-action authoring flow: attach an existing UI cleanup rule, choose one bounded action, optionally filter to a keyword or structural trigger, optionally constrain editable-field trigger location, and decide whether the action should be followed by normal blocking.
