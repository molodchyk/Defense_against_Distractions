...
- Triggered action chains now have a non-mutating current-page preview message and popup Inspect preview panel that report bounded scenario, target, selected-step, trigger-diagnostics, and would-block status without running actions or exposing page text.
...
- DaD Select quick add: current-page snapshots can include a bounded active selected-text candidate, and the new selection-only right-click menu item can hand selected text to the popup as a short-lived local pending candidate. The popup can show/copy or save that candidate as a keyword rule in an existing or new current-site plan entry, with editable score, optional bounded cleanup presets, and an action-chain preset that opens the selected plan's Actions editor after the keyword is saved or confirmed, preselecting the saved keyword trigger when the compact editor can represent it.
