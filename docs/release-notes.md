...
- Plans now expose an `Actions` tab for the first compact triggered-action authoring flow: attach one or two existing UI cleanup rules as ordered bounded actions, optionally filter to a keyword or structural trigger, optionally constrain editable-field trigger location, and decide whether the action sequence should be followed by normal blocking.
- Popup Block Diagnostics now shows the latest triggered-action outcomes as bounded action/result pairs, so action cleanup and fallback blocking are visible without exposing page text, selectors, or full URLs.
