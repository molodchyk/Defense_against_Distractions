...
import { getStoreAutomationFailures } from './playbook/store/automation.mjs';
import { getStoreMediaReviewCoverageFailures, verifyReviewedStoreMediaHashes } from './playbook/store/mediaReview.mjs';
...
for (const locale of locales) {
  ...
  if (englishStoreListingLocales.has(locale)) {
    assertCondition(
      /plans/i.test(listing) && /allowed websites/i.test(listing) && /intent coherence/i.test(listing) && /processed locally/i.test(listing) && /remote server/i.test(listing) && /You can use it to protect/i.test(listing) && /Main features:/i.test(listing) && /Browser notes:/i.test(listing) && /What is new in version/i.test(listing) && /incognito/i.test(listing) && /file URL/i.test(listing) && /open source/i.test(listing),
      `${listingPath} must describe examples, features, privacy boundary, current plan-based UI model, browser-controlled limitations, current-version notes, and open-source footer.`
    );
    assertCondition(
