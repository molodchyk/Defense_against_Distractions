...
Run `npm run verify:playbook` to check README, privacy, license, package metadata, and store-listing alignment against the shared browser-extension playbook. Run `npm run verify:package` after packaging to check the generated runtime output for manifest/import consistency, source-map policy, and remote executable code. Run `npm run verify:release` to check the generated archives and the package gate together. See [Release Readiness](docs/release-readiness.md) for the full automated and manual release gates.
